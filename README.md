...
- **Reuse:** The agent invokes these skills in later loops, working better and staying consistent with the human's prior decisions. **It must also check `docs/knowledge-asset/published/`** — files there are already approved and ready to use, not drafts awaiting a decision; a human still needs to physically create the matching `.claude/skills/` file (see the write-guard workaround), but that's a mechanical copy step, not an approval gate. Every turn's comment states, in one line, which `published/` file(s) it consulted and applied (or that none applied). Files that go stale move to `docs/knowledge-asset/deprecated/` and are no longer applied.
...
| Branch | Meaning |
|---|---|
| feature branch | Where the AI opens the Test PR / Code PR for each loop — always with an explicit `--base develop` (never rely on the default base branch, which may be `main`), including any manually-posted "Create PR" compare link before a PR exists (`compare/develop...branch`, never `compare/main...branch`). The AI runs the `origin/develop` sync check automatically and unconditionally as the first action of every `@claude approved` turn (and before editing anything more generally), never skipped based on an assumption that the branch "looks fine". For a brand-new Test PR / Code PR branch with no pushed remote history yet, the reset to `origin/develop` is unconditional — it does not wait for a content diff to show a difference, since a locally-continued commit chain from an already-merged issue can look identical in content while still being stale (see issue #106 — jobs sometimes check out `main` instead of `develop`; issue #305 — stale-but-identical-content history produced a noisy PR diff later). A branch with its own pushed commits (follow-up work on an open PR) is never force-reset. Once its PR is open, post follow-ups on that PR (not the parent issue), otherwise the harness spins up a stray duplicate branch. Before re-implementing an already-approved change, check for an existing branch/PR with the same diff and reuse it instead of starting over |
