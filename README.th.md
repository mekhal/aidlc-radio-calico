...
- **ขึ้นต้นทุกคอมเมนต์ด้วย TL;DR** ทุกคอมเมนต์ที่มีเนื้อหาสำคัญต้องขึ้นต้นด้วย **1. Done** (สิ่งที่ทำเสร็จแล้ว), **2. Scope** (สิ่งที่จะทำต่อ), **3. Action Required** (สิ่งที่ต้องการให้คนตัดสินใจ) ก่อนรายละเอียดอื่น ๆ — รายละเอียดภายใน (ใช้ gate/skill/rule ไหน) ไม่ต้องใส่ในสรุปที่มนุษย์อ่าน ให้เก็บไว้ใน `docs/decisions/`/`ai-review-evals/` แทน
...
- **เรียกใช้ (Reuse):** agent เรียก skill เหล่านี้ในรอบถัดไป ทำให้ทำงานได้ดีขึ้นและสอดคล้องกับการตัดสินใจเดิมของมนุษย์ **ต้องตรวจ `docs/knowledge-asset/published/` ด้วยเสมอ** — ไฟล์ที่อยู่ใน `published/` ถือว่า**ผ่านการอนุมัติและพร้อมใช้งานแล้ว** ไม่ใช่ฉบับร่างที่รอการตัดสินใจ เหตุผลเดียวที่ไฟล์ยังไม่ถูกย้ายเข้า `.claude/skills/` คือขั้นตอนกลไกตาม write-guard (ต้องให้มนุษย์เป็นผู้สร้างไฟล์ `.claude/skills/<kebab-name>/SKILL.md` เอง) ไม่ใช่การรออนุมัติ จึงต้องอ่านและนำแนวทางใน `published/` มาใช้เหมือนกับ skill ที่อยู่ใน `.claude/skills/` แล้ว ทุกคอมเมนต์ในแต่ละรอบต้องระบุสั้น ๆ ว่าปรึกษาและนำไฟล์ใดใน `published/` มาใช้ (หรือระบุว่าไม่มีไฟล์ที่เกี่ยวข้อง) ไฟล์ที่ล้าสมัยแล้วจะถูกย้ายไปที่ `docs/knowledge-asset/deprecated/` และไม่ถูกนำมาใช้อีก
...
| Branch | ความหมาย |
|---|---|
| feature branch | ที่ AI เปิด Test PR / Code PR ในแต่ละ loop — ต้องระบุ `--base develop` ชัดเจนเสมอ (ห้ามพึ่ง default base branch ซึ่งอาจเป็น `main`) รวมถึงลิงก์ "Create PR" ที่พิมพ์เองในคอมเมนต์ก่อนมี PR จริง (`compare/develop...branch` ห้ามใช้ `compare/main...branch`) AI ต้องรัน sync check กับ `origin/develop` โดยอัตโนมัติและไม่มีเงื่อนไข เป็นการกระทำแรกของทุกครั้งที่มีคำสั่ง `@claude approved` (และก่อนแก้ไฟล์ใด ๆ โดยทั่วไป) ห้ามข้ามเพราะคิดเอาเองว่า branch "ดูปกติดี" — สำหรับ branch ใหม่ของ Test PR / Code PR ที่ยังไม่มีประวัติบน remote ต้อง reset ไปที่ `origin/develop` เสมอโดยไม่มีเงื่อนไข ไม่ต้องรอให้ content diff ต่างกันก่อนถึงจะ reset เพราะ commit chain ที่สืบทอดมาจาก issue เก่าที่ merge ไปแล้วอาจมี content เหมือนกันแต่ยังคง stale อยู่ (ดู issue #106 — บาง job checkout ไปที่ `main` แทน `develop`; issue #305 — ประวัติ stale ที่ content เหมือนเดิมทำให้ PR diff รกในภายหลัง) ส่วน branch ที่มี commit ที่ push ไปแล้ว (งานต่อเนื่องบน PR ที่เปิดอยู่) จะไม่ถูก force-reset เด็ดขาด เมื่อ PR เปิดแล้ว ให้คอมเมนต์ตามงานต่อที่ตัว PR นั้น (ไม่ใช่ที่ issue ต้นทาง) ไม่เช่นนั้น harness จะสร้าง branch ซ้ำซ้อนขึ้นมาใหม่โดยไม่ตั้งใจ ก่อนทำงานเดิมซ้ำที่เคยอนุมัติแล้ว ให้เช็คว่ามี branch/PR เดิมที่ diff เดียวกันอยู่แล้วหรือไม่ แล้วนำมาใช้ต่อแทนการเริ่มใหม่ |
